...
| Security & Supply-Chain | Applies — **ASVS L2** | Merge-blocking pip-audit, gitleaks, CodeQL, zizmor, Trivy, Semgrep, OSV, and no-outing gates — all thirteen in the live ruleset's required-check set (see [`.github/rulesets/main.json`](.github/rulesets/main.json)); SHA-pinned Actions; digest-pinned container base; SBOM, keyless signing, and SLSA provenance on tagged releases. |
| CI/CD | Applies | Least-privilege workflow tokens, SHA-pinned actions, Harden-Runner on every job (16 of 24 in `egress-policy: block`; the 8 `release.yml` jobs stay in `audit` until a first real release supplies an observed allowlist, [#78](https://github.com/ChelseaKR/ledger/issues/78)), a live `main` ruleset mirrored in-tree at [`.github/rulesets/main.json`](.github/rulesets/main.json) — pull-request-required, signed, linear-history, strict-checks, thirteen-context solo-maintainer profile per [`.github/rulesets/README.md`](.github/rulesets/README.md) — and local `make verify` parity with the portable merge gates. |
...
| Internationalization | Applies | Packaged gettext catalogs ship complete EN/ES/FR/AR translations, including Arabic RTL, with blocking extraction, BCP-47, parity, completeness, compilation, pseudolocale, and response-header checks. **The ES/FR/AR catalogs are drafted, not reviewed** — no qualified speaker has read them, the parity gates cannot tell the difference (a verbatim-English `msgstr` passes all three), so every non-English page, every non-English `.po` header, and [`docs/I18N.md`](docs/I18N.md#translation-review-drafted-not-reviewed) say so. |
