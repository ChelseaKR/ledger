"""Hashing and checksum verification — the integrity floor of the preservation core.
...
def verify_file(path: Path, algo: HashAlgo, expected: str) -> FixityResult:
    ...
    Returns a :class:`~ledger_preservation_core.models.FixityResult` rather than raising, so a
...
class AuditReport:
    ...
    Carries every individual :class:`~ledger_preservation_core.models.FixityResult` so a steward
