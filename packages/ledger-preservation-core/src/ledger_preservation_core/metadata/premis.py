"""PREMIS event log — an append-only record of what happened to preserved content.
...
keeps an *append-only* sequence of
:class:`~ledger_preservation_core.models.PremisEvent` objects and serializes it
two ways:

* canonical JSON — a durable form, byte-stable for hashing;
...
* **Interoperability / standards-compliance.** :func:`to_premis_xml` emits valid
  ``premis:premis`` markup so another repository can read this event history.

No-outing rule: a :class:`~ledger_preservation_core.models.PremisEvent` carries an
*agent*, an *outcome*, a *detail*, and an opaque *linked_object* (a content
address, record id, or bag id) — never a contributor identity or a sealed value.
This module adds nothing to that shape; it only orders, serializes, and persists it.
"""
...
from ledger_preservation_core.models import PremisEvent, PremisEventType, canonical_json
