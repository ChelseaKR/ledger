...
This module serializes a :class:`~ledger_preservation_core.models.DublinCore` to
and from a canonical JSON sidecar, and exports the standard ``oai_dc:dc`` XML
form used by OAI-PMH harvesters.
...
No-outing rule (inherited from ledger, this library's first consumer):
``dc.creator`` and ``dc.contributor`` should describe the *collection* or
*community* that holds an item — never a closeted individual who contributed
it. A caller whose domain has a notion of protected identity (as ledger's own
contributor model does) must keep it out of Dublin Core entirely and resolve
it only behind an explicit access grant elsewhere; it should never be written
to a discoverable sidecar or XML record. This module trusts that the values
handed to it already honour that rule and adds no identity of its own.
"""
...
from ledger_preservation_core.models import DC_ELEMENTS, DublinCore, canonical_json
