...
.PHONY: help venv install lint format type test cov audit accessibility acr demo serve \
        i18n i18n-compile secret-scan workflow-lint container verify clean
...
# target comment.)
verify: lint type test i18n accessibility audit secret-scan workflow-lint ## Run the complete merge gate (== CI's required checks)
