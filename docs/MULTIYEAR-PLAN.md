...
| MP-05 | **Partly built** | The coverage half is done (90.57%) and both public route tables are refactored together, whole, as this row required. Six `C901` waivers remain: `ingest_sip`, `validate_bag`, WCAG element/rule checks, CLI ingest options, and untrusted-form validation. `ROADMAP.md` calls it "real, non-mechanical work on the repo's most security-critical paths" and it is. Nothing blocks the rest but size |
