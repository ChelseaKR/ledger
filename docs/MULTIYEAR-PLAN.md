...
|---|---|---|---|---|
| MP-01 ✅ | **The recorded reason becomes durable.** `ModerationLogStore`, every live write path, two steward-gated read surfaces, ADR 0014 | P0 | L | [#156](https://github.com/ChelseaKR/ledger/issues/156); `GOVERNANCE.md`, `THREAT-MODEL.md` §4.4 · **shipped, see below** |
| MP-02 | **`TombstoneStore` gets the file lock every sibling store has.** ~85% of tombstones are lost at 40 concurrent takedowns; a lost tombstone means a reattaching replica is never told to delete its stale copy | P0 | S | [#155](https://github.com/ChelseaKR/ledger/issues/155); Hard Rule 4 · **[corroborates README Hard rules]** |
| MP-03 | **`ProposalStore` and `SubmissionQueue` stop reading corruption as an empty history.** Both swallow `(OSError, ValueError)` and return `[]`, and the next `add()` then rewrites the file | P0 | S | [#154](https://github.com/ChelseaKR/ledger/issues/154) · **[corroborates ADR 0014 decision 1]** |
...
**Done when:** 40 concurrent takedowns record 40 tombstones; a truncated `proposals.json`
raises instead of reading as an empty history; every module in the security core reports
against a floor of its own with no pooled `--include` remaining in `make cov`; ADR 0006
carries `Superseded by 0009` and 0009 points back; `DEFINITION_OF_DONE.md` names thirteen
required checks.
...
## Implementation status — 2026-08-27 (MP-01)

**MP-01 shipped.** `moderate.ModerationLogStore` persists the log at
`<store>/logs/moderation.json`; `ledger policy` / `seal` / `cw` / `takedown`, an executed
dual-control `publish`, the steward console's warn / takedown / submission review, and a
contributor's own withdrawal each record their decision; `/steward/audit` and
`ledger moderation list|verify` read it back. Recorded as ADR 0014.

Verify: `make verify` green (1257 tests); `make cov` total 89.01% against the 88% floor,
the pooled scope unchanged at 95%, `moderate.py` 90% against a floor of its own. 28 new
tests, of which two run unchanged against `origin/main` and fail there.

Nothing else in this plan is built. MP-02 through MP-14 are planned only.
