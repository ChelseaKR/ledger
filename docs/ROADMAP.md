...
| Release & Versioning | REL-03/08/17/20 | Release workflow exists and the **repo side is now complete and gated**: the approved signer's key is committed at `.github/allowed_signers`, `release.yml` verifies every release tag against it, and `tests/test_release_readiness.py` fails the build if that list is emptied or gains a second signer, if the publish job loses its `pypi` environment scoping or gains a stored token, or if the Trusted Publisher values the runbook tells a human to type stop matching what the workflow declares. What remains is owner-only: the PyPI project registration, the Trusted Publisher form, the `pypi` GitHub Environment, and the first end-to-end release | Open — [#80](https://github.com/ChelseaKR/ledger/issues/80) | First signed tag publishes and verifies successfully with an approved signer |
...
| Code Quality | CQ-05/08/34/35 | Suppression hygiene is a blocking gate (`make hygiene`, `tools/check_hygiene.py`) and every suppression is coded, explained, and issue-linked where temporary; the 6 remaining `C901` complexity waivers link #83. The pooled scoped floor is gone: `tools/check_coverage_floors.py` now gates each of the 8 security-core modules on its own measured value, and refuses both an unfloored security-core module and a floor naming a module that no longer exists (ADR 0015). The global branch-coverage floor now stands at **CQ-08's published-library target of 90%** (ratcheted 85% → 88% → 90%, measured 90.13% on 2026-09-01 and 90.57% on 2026-09-06), reached with real tests rather than a lowered bar: `transparency.py` went 84% → 100% by exercising every rejection path of the warrant canary, and the remaining points came from the refusal branches of `fixity`, `metadata.pid`, `metadata.dublincore`, `metadata.ead`, `oais`, `upload` and `succession`. What is still open here is the other half. Two of the eight are done: the public GET and POST route tables (`do_GET`, complexity 35, and `do_POST`, 12) are now declared route tables that `tests/test_route_tables.py` drives over a live server, asserting every path reaches the handler and the arguments it reached before — and the split also closed a gate that could not fail, since `tests/test_accessibility_route_coverage.py` had been recovering the route inventory by regular expression over `do_GET`'s source text and passes a set difference against an empty set. The 6 that remain — the SIP ingest pipeline, BagIt validation, WCAG element/rule checks, CLI ingest option handling, and untrusted-form field validation — are still waived rather than refactored. Each is an independent-branch sequence with a documented reason a mechanical split would make worse, not oversight; refactoring the ingest pipeline with safety-preserving tests is real, non-mechanical work on the repo's most security-critical paths | Open — [#83](https://github.com/ChelseaKR/ledger/issues/83) | Branch coverage reaches 90% — **done, 90.13% measured 2026-09-01** — and the 8 `C901` waivers are refactored away rather than tracked — 2 done (the GET and POST route tables), 6 to go |
...
| `make verify` portable gate | Green: lint, strict types, the full test suite, i18n, structural accessibility, ACR check, dependency/secret scans, Semgrep SAST (when the binary is present), truthfulness, suppression hygiene, zizmor | `make verify` | 2026-09-01 |
