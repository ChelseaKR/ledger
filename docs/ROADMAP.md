...
| Security & Supply-Chain / CI/CD | SEC-11/13, CICD-27 | pip-audit and OSV-Scanner over `uv.lock` are blocking (ci.yml `supply-chain` + `osv` jobs, mirrored by `make audit`/`make osv`), but local Semgrep parity is not yet in `make verify` | Open — [#84](https://github.com/ChelseaKR/ledger/issues/84) | Add locked Semgrep tooling and preserve fail-closed local/CI parity |
