...
| RM6 ✅ | **Captions/transcripts as a first-class ingest step** for audio/video oral histories | E2, A3, D2 | P1 | M | `ACCESSIBILITY.md` (captions/transcripts "where source allows"); ACR. **[corroborates ACCESSIBILITY]** — *shipped in `a4b9f28`: `ledger.captions` parses WebVTT and SRT (format sniffed from the bytes, never the extension), `ledger ingest --captions` attaches the cues with their real segment/timing structure, and `render` surfaces them as a timed transcript. ledger still does **no** transcription: a caption file is something a human produced, as `docs/ACCESSIBILITY.md` requires.* |
...
| RM11 ✅ | **Committed cadence of manual NVDA/VoiceOver review** (`docs/accessibility/MANUAL-REVIEW-CADENCE.md`) + browser-real **axe CI** over the served site (`tools/a11y_browser/`, `accessibility-browser` job) as engine-backed depth over the static gate (FIX-12), **+ `aria-live` announcement of every dynamic status message (SC 4.1.3)**, enforced in both directions by the static gate and pinned by `tests/test_aria_live_status.py`. | E2 | P3 | S | ACR: automated gate is a *floor*, manual review periodic. **[corroborates ACCESSIBILITY]** — *done: `render._status_region`, `accessibility_check._STATUS_CLASSES`; ACR 4.1.3 cites the gate.* |
...
| EX3 | ✅ **Map/place + timeline browse with the list/table non-visual equivalent** | D1, D2, A4 | P2 | M | `ACCESSIBILITY.md` explicitly anticipates "where a map view is later added." **[corroborates ACCESSIBILITY (anticipated)]** — **DONE**: `/places` (DC `coverage`) + `/timeline` (DC `date`, list+table), `coverage=` facet composes with the rest. |
| EX4 | ✅ **Relationships between records** (zine ↔ maker ↔ oral history) via PREMIS/DC relations + accessible, list-equivalent graph | D1, A4 | P2 | M | Scholarly contextualization; DC/PREMIS relation support. **[NET-NEW]** — **DONE**: record-detail "Linked records" resolves DC `relation` (+ reciprocal) against disclosed ids only; a relation to a sealed record renders as nothing. |
...
Shipped since (2026-07-02): **RM11 + FIX-12** accessibility verification depth — a browser-real Playwright + axe-core CI job (`accessibility-browser`) drives the served demo in headless Chromium under light and dark schemes across the ~8 canonical pages (browse, search/facets, the CW-interstitial record view, contribute, steward console), plus a keyboard-traversal spec; and a committed manual NVDA/VoiceOver review cadence (`docs/accessibility/MANUAL-REVIEW-CADENCE.md`) now backs the ACR's stated evidence basis. All new deps are CI/dev-only under `tools/a11y_browser/` — the stdlib-only runtime is unchanged. The axe run also surfaced and fixed a real dark-mode button/skip-link contrast defect (white text on the light accent, 2.4:1) by taking the button foreground from the already-audited `--bg` token. Still owed under RM11 at the time: `aria-live` announcement of status messages (ACR 4.1.3).

**RM11 completed (2026-09-05): `aria-live` announcement of status messages (SC 4.1.3).** The audit found the browse/search result count already in a polite live region and the CW interstitial already `role="alert"`, and six real gaps behind them — the `/overview` total and empty state, the `/places` and `/timeline` empty states, and the steward console's empty submission and request queues, all rendered outside any live region and therefore never spoken. One `render._status_region` helper now emits the region (scoped to the message paragraph and nothing else) at every call site; a rejected submission and an unreadable steward queue take the assertive register instead. The rule is enforced, not asserted: `ledger.accessibility_check` fails the build on a status message outside a live region **and** on a region scoped wider than its message — an over-broad `aria-live` re-announces page structure until a reader tunes it out, so the second half is not optional. `tests/test_aria_live_status.py` drives every dynamic state (populated/empty, clean/rejected, plus the two console surfaces that need a live server) with negative controls that remove and widen the region and assert the checker still catches it. Also corrected: `docs/ACCESSIBILITY.md` had gone stale against its own generated ACR, still describing 1.4.3/1.4.11 and 4.1.3 as `Partially Supports` after `acr_gen.py` had raised all three to `Supports`.
...
Shipped this pass: **EX3** map/place + timeline browse — new `/places` view (Dublin Core `coverage`, a heading-structured place list linking into a composed `?coverage=` facet that AND-composes with `subject`/`type`/`language`) and `/timeline` view (Dublin Core `date` grouped by year, rendered as an ordered list *and* a table equivalent), both list/table-first with no external tiles or JS (`search.facet_by_coverage`/`group_by_year`, `render._places_html`/`_timeline_html`) · **EX4** record relationships — a record-detail "Linked records" section that resolves DC `relation` values (and the reciprocal "records that link here") against *only* disclosed record ids, shows external identifiers as plain text, and renders a relation to a sealed/withheld record as nothing, never leaking its existence (`search.resolve_relations`, `render._relations_html`).
